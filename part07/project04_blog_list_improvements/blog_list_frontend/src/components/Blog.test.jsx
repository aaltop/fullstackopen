...
describe("Blog element", () => {
    ...
    const shownByDefault = [testBlog.title, testBlog.author]

    const notShownByDefault = [testBlog.url, testBlog.likes]
    ...
    function toggleVerbose() {
        const button =
            screen.queryByText("Show") ??
            screen.queryByText("Hide")
    ...
    test("renders only title and author by default", () => {
        ...
        notShownByDefault.forEach(val => {
            expect(
                screen.queryByText(val, { exact: false })
            ).toBeNull()
        })
    })
    ...
    test("renders expected values after 'Show' click", () => {
        ...
        const shownInVerbose = shownByDefault.concat(
            notShownByDefault
        )
